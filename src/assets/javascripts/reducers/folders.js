...
export default function folders(state = { byID: [], byHash: {}, editing: null }, action = {}) {
    switch (action.type) {
        case ADD_FOLDER: { // also can add an array of f because it uses concat
            let byID = state.byID.slice(0);
            let byHash = { ...state.byHash };

            const folder = Object.assign({}, action.folder, {
                id: folderID++, // give new folder an ID
                stubs: [],
                open: true
            });
        ...
        case ADD_STUB: {
            const folder = state.byHash[action.folderID];
            const byHash = {
                ...state.byHash,
                [action.folderID]: {
                    ...folder,
        ...
        case MOVE_STUB: {
            const byHash = { ...state.byHash };
            const oldFolder = byHash[action.prevFolder];
            const newFolder = byHash[action.nextFolder];
            let stubs = newFolder.stubs.slice(0);

            // if it changed folders, we remove it from the old one
            if (action.prevFolder !== action.nextFolder) {
                byHash[oldFolder.id] = {
                    ...oldFolder,
                    stubs: oldFolder.stubs.filter(stubID => stubID !== action.stub.id)
                };
                // add it to the new one
                if (action.position === undefined) {
                    stubs.push(action.stub.id);
                } else {
                    stubs.splice(action.position, 0, action.stub.id);
                }
            } else {
                // otherwise we're moving it in its current folder
                const currentPosition = stubs.indexOf(action.stub.id);
                ...
                if (action.position !== currentPosition && action.position !== currentPosition + 1) {
                    ...
                    stubs = stubs.filter(stubID => stubID !== action.stub.id);
                    stubs.splice(newPos, 0, action.stub.id);
                } else {
...
export const moveStub = (prevFolder, nextFolder, stub, position) => ({
    type: MOVE_STUB,
    prevFolder,
    nextFolder,
    stub,
...
export const folderActions = {
    addFolder, removeFolder, editFolder, toggleFolderOpen, moveStub, moveFolder
};
